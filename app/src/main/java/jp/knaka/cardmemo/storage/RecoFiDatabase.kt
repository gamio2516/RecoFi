    ...
    companion object {
        ...
        val MIGRATION_4_5 = object : Migration(4, 5) {
            override fun migrate(db: SupportSQLiteDatabase) {
                ...
                db.execSQL("CREATE TABLE recurring_price_revisions_new (recurringId INTEGER NOT NULL, effectiveDate TEXT NOT NULL, amount INTEGER NOT NULL, PRIMARY KEY(recurringId,effectiveDate), FOREIGN KEY(recurringId) REFERENCES recurring_expenses(id) ON DELETE CASCADE)")
                ...
                val foreignKeyViolations = db.query("PRAGMA foreign_key_check").use { cursor ->
                    buildList {
                        while (cursor.moveToNext()) {
                            add("table=${cursor.getString(0)}, rowid=${cursor.getLong(1)}, parent=${cursor.getString(2)}, fkid=${cursor.getInt(3)}")
                        }
                    }
                }
                check(foreignKeyViolations.isEmpty()) { "外部キー整合性を確認できませんでした: ${foreignKeyViolations.joinToString()}" }
            }
        }
