...
class BackupManagerRoomTest {
 ...
 @Test fun invalidBackupCannotChangeRoom(){val db=StorageProvider.database(context);seed(db);val repo=TransactionRepository(context);repo.save(listOf(Transaction(1,100L,"食料品","safe","keep",1L,paymentSourceId="rakuten")));val valid=BackupCodec.decodeAndValidate(BackupCodec.encode(db,"1.0"));val invalid=valid.copy(snapshot=valid.snapshot.copy(transactions=listOf(valid.snapshot.transactions.single().copy(categoryId="missing"))));Assert.assertTrue(runCatching{BackupManager(context).restore(invalid)}.isFailure);Assert.assertEquals("safe",repo.load().single().merchant)}
 private fun seed(db:RecoFiDatabase){db.referenceData().upsertPaymentSources(listOf(PaymentSourceEntity("rakuten","楽天カード","CREDIT_CARD",0)));db.referenceData().upsertCategories(listOf(CategoryEntity(stableCategoryId("食料品"),"食料品",0)));db.monthlyState().upsertBudgetSettings(AppBudgetSettingsEntity(defaultMonthlyBudget=0L))}
}
