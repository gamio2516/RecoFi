...
@RunWith(RobolectricTestRunner::class) class BackupCodecTest{
 ...
 @Test fun v4RoundTripPreservesTransactionMatchAndDefaultSource(){val d=BackupCodec.decodeAndValidate(encoded());assertEquals("西友",d.snapshot.transactions.single().merchant);assertEquals("CONFIRMED",d.snapshot.matches.single().status);assertEquals("card",d.snapshot.budgetSettings.defaultPaymentSourceId);assertEquals(4,JSONObject(encoded()).getInt("backupFormatVersion"))}
