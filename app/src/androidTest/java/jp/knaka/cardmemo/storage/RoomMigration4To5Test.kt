...
class RoomMigration4To5Test {
    ...
    @Test fun preservesCategoriesTransactionsRecurringAndReferences(){
        ...
        assertEquals(category.id,room.transactions().loadAll().single().categoryId)
        ...
        assertEquals(1280L,room.recurringExpenses().loadRevisions().single().amount)
        assertEquals("CONFIRMED",room.reconciliation().loadMatches().single().status)
        room.close()
